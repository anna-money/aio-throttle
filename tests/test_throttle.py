...
from aio_throttle import Throttler, ThrottleRequest, ThrottleStats, defaultdict
from aio_throttle.quotas import StaticConsumerQuota
...
def stats(available_capacity, capacity_limit, queue_size, queue_limit, by_consumers=None):
    return ThrottleStats(
        available_capacity, capacity_limit, queue_size, queue_limit, defaultdict(int, by_consumers or {})
    )
...
async def test_throttle_without_queue():
    ...
    assert throttler.stats == stats(1, 1, 0, 0)
    async with throttler.throttle() as t1:
        assert t1.accepted
        assert throttler.stats == stats(0, 1, 0, 0)
...
async def test_throttle_with_rejecting_consumer_quota():
    ...
    assert throttler.stats == stats(1, 1, 0, 1, {})
    async with throttler.throttle(request) as t1:
        assert not t1.accepted
...
async def test_throttle_with_queue():
    ...
    assert t1.accepted
    assert throttler.stats == stats(0, 1, 0, 1, {})
    ...
    assert throttler.stats == stats(0, 1, 1, 1, {})
